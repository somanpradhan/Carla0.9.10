...
class World(object):
    ...
    def render(self, display):
        ...
        state, labelConf = self.camera_manager.render(display)
        self.hud.render(self, display)
        return state, labelConf  
...
class CameraManager(object):
    ...
    def render(self, display):
        ...
        if self.surface is not None:
            ...
            if self.labelconf:
                return self.state, self.labelconf
            else: 
                return self.state, []
    ...
    def _parse_image(weak_self, image):
        ...
        if self is None:
            return
        self.surface,self.state, self.labelconf = DetectingObject.parse_image(image)
...
# for the implementing the agent code
        ...
        while True:
            ...
            state, labelConf= world.render(display)
            pygame.display.flip()
            ...
            some_state = False
            labels = labelConf[0] if labelConf else []
            confs = labelConf[1] if labelConf else []
...
#        Changing the desired speed based on Sign Detected
            ...
            for index in range(len(labels)):
                log_event("Testing", {
                    "sign": labels[index],
                    "pedestrian_detected": p_state,
                    "sign_conidence": confs[index] ,
                    "speedvehicle_speed": desired_speed * 3.6,
                    "vechicle_state": True
                })
...
# Using Behaviour agent
        ...
        print(f"[LOG SAVED] Total Events: {len(log_data['events'])}")
